...
class sim:
    ...
    def __init__(self, state=None, side=8, seed=8, gpu=False, gpu_select=0, warp=8, spawnStabilityFactor=-1):
        # Validate input.
        ...
        self.stable = np.zeros(self.size, dtype=np.int32)           # Used to store stable values for each cell, NOTE: IS SIGNED!
        ...
        if gpu:
            ...
            __global__ void run(const unsigned char* world, unsigned char* result, int* stable, const unsigned int side, const unsigned int size)
            {{
                ...
                {{
                    ...
                    stable[cellLoc] = (currState * prevState * stable[cellLoc] + prevState * currState) + ({} * currState * (!prevState));
                }}
            }}
            """.format(self.spawnStabilityFactor))
...
# These are private functions that should not be mixed or used outside of this class.
    ...
    def __step_state_cpu(self):
        for cellLoc in range(self.size):
            ...
            self.stable[cellLoc] = (currState * prevState * self.stable[cellLoc] + prevState * currState) + (self.spawnStabilityFactor * currState * (not prevState))
...
# --- I/O ---
    # Will either return a vector OR a 2D square matrix of the system.
    def get_state(self, vector=False):
        return self.world if vector else self.world.reshape((self.side, self.side))

    # Will either return a vector OR a 2D square matrix of the stables of each cell with respect to their position.
    def get_stable(self, vector=False):
        return self.stable if vector else self.stable.reshape((self.side, self.side))
    ...
    def save(self):
        return (self.world, self.stable, self.side, self.count, self.spawnStabilityFactor)

    # Load from memory an exact state setup.
    def load(self, newState, newstable, side, count, spawnStabilityFactor):
