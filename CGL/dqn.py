...
class DQNAgent():
    ...
    def select_action(self, state, epsilon=0.):
        ...
        Q_all = self.Q(torch.tensor(state, dtype=torch.float32))

        
        greedy_action = None
        greedy_action_value = None
        if chance > epsilon : 
            # enter into maxQt(a)
            for a in range(len(Q_all)):
                if greedy_action is None:
                    greedy_action = a
                    greedy_action_value = Q_all[a]
                else:
                    if greedy_action_value < Q_all[a]:
                        greedy_action = a
                        greedy_action_value = Q_all[a]
            ...
            choice = np.random.choice(np.delete(np.arange(len(Q_all)), greedy_action))
            
            return choice
        
    
    def max_action(self, state, Q):
        """
        custom function return max action 
        """
        ###### TYPE YOUR CODE HERE ######
        ################################# 
        Q_all = Q(torch.tensor(state, dtype=torch.float32))
        greedy_action = None
        greedy_action_value = None
        # enter into maxQt(a)
        for a in range(len(Q_all)):
    ...
    def learn(self, experiences, discount):
        ...
        #print(f"Q_values : {Q_values.size()}")
        #print(f"target_Q_values : {target_Q_values.size()}")
        #quit()
        ...
        # next time I use this vector treat it as a constant vector with no wgreadient update
...
if __name__ == "__main__":
    ...
    for e in range(args.n_episodes):
        ...
        print(f"epsilon : {epsilon} , epsilon decay : {epsilon_decay}")
        
        
        if above_200:
            if np.mean(moving_window) > 200:
                    if times_above_200 >= 100:
                        break
                    times_above_200 += 1
