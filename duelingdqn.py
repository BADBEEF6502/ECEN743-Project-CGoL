...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, net_mul):
        ...
        halv=int(hidden/2)
        self.V1 = nn.Linear(hidden, halv)
        self.V2 = nn.Linear(halv, 1)
        
        self.A1 = nn.Linear(hidden, halv)
        self.A2 = nn.Linear(halv, action_dim)
...
class duelingDQNAgent():
    ...
    def learn(self, experiences, discount):
        ...
        with torch.no_grad():
            Q_action=  self.Q(next_states).detach().max(axis=1)[1].unsqueeze(1)
            prev_Q=self.Q_target(next_states).gather(1, Q_action)
        #target_Q_values=rewards+discount*prev_Q
        target_Q_values = torch.add(rewards, torch.mul(discount, prev_Q))
