...
#    parser.add_argument("--name", help='Used for HPRC applications with same side seed.')                                                       # HPRC specific parameter.
    ...
    learner = duelingDQNAgent(**kwargs)
    ...
    for e in range(args.n_episodes):        # Run for some number of episodes.
        ...
        if e % args.max_eps_len == 0 and e != 0:
            ...
            data_alive.append(np.mean(num_alive))
            data_still_life.append(still_life_count)
    ...
    # Save the data files.
    name = f'{args.side}_{args.discount}_dueling'
    ...
    data2save = {'breakdown' : data_breakdown, 'evals' : data_evals, 'rewards' : data_rewards,
                 'data_heatmaps' : data_heatmaps, 'data_actions' : data_actions, 
                 'data_alive' : data_alive,
                 'data_still_life' : data_still_life,
                 'eval_period' : args.max_eps_len}
    with open(f'data_{name}_dueling.pkl', 'wb') as f:
